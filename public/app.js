document.addEventListener('DOMContentLoaded', () => {
  ...
  function startTwoPlayer() {
    ...
    socket.on('player-connection', (num) => {
      console.log(`${num} számú játékos csatlakozott vagy kilépett.`);
    ...
    socket.on('player-number', (num) => {
      if (num === -1) {
        infoDisplay.innerHTML = 'Elnézést, jelenleg a szerver tele';
      } else {
        ...
        startButton.addEventListener('click', () => {
          if (allShipsPlaced) {
            ...
            infoDisplay.innerHTML = 'Kérlek, helyezd el az összes hajót';
          }
        });
      }
    });
    ...
    socket.on('timeout', () => {
      // return to main menu
      infoDisplay.innerHTML = 'Elérted a 10 perc játék limit időt';
    });
  }
      ...
      if (
        ...
        for (let i = 0; i < draggedShipLength; i++) {
          ...
          if (isHorizontal) {
            ...
            square =
              userSquares[parseInt(this.dataset.id) + width * i - offset];
  ...
  function revealSquare(classList) {
    ...
    if (
      ...
      if (obj.includes('romboló')) {
        destroyerCount++;
      }
      if (obj.includes('tengeralattjáró')) {
        submarineCount++;
      }
      if (obj.includes('cirkáló')) {
        cruiserCount++;
      }
      if (obj.includes('hadihajó')) {
        battleshipCount++;
      }
      if (obj.includes('repülőgép-hordozó')) {
        carrierCount++;
      }
  ...
  function checkForWins() {
    if (destroyerCount === 2) {
      infoDisplay.innerHTML = 'Elsüllyesztetted az ellenfél rombolóját';
      destroyerCount = 10;
    }
    if (submarineCount === 3) {
      infoDisplay.innerHTML = 'Elsüllyesztetted az ellenfél tengeralattjáróját';
      submarineCount = 10;
    }
    if (cruiserCount === 3) {
      infoDisplay.innerHTML = 'Elsüllyesztetted az ellenfél cirkálóját';
      cruiserCount = 10;
    }
    if (battleshipCount === 4) {
      infoDisplay.innerHTML = 'Elsüllyesztetted az ellenfél hadihajóját';
      battleshipCount = 10;
    }
    if (carrierCount === 5) {
      infoDisplay.innerHTML =
        'Elsüllyesztetted az ellenfél repülőgép-hordozóját';
      carrierCount = 10;
    }
    if (enemyDestroyerCount === 2) {
      infoDisplay.innerHTML = 'Az ellenfél elsüllyesztette a rombolódat';
      enemyDestroyerCount = 10;
    }
    if (enemySubmarineCount === 3) {
      infoDisplay.innerHTML =
        'Az ellenfél elsüllyesztette a tengeralattjáródat';
      enemySubmarineCount = 10;
    }
    if (enemyCruiserCount === 3) {
      infoDisplay.innerHTML = 'Az ellenfél elsüllyesztette a cirkálódat';
      enemyCruiserCount = 10;
    }
    if (enemyBattleshipCount === 4) {
      infoDisplay.innerHTML = 'Az ellenfél elsüllyesztette a hadihajódat';
      enemyBattleshipCount = 10;
    }
    if (enemyCarrierCount === 5) {
      infoDisplay.innerHTML =
        'Az ellenfél elsüllyesztette a repülőgép-hordozódat';
